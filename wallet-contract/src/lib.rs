...
pub trait Wallet {
    ...
    /// Executes a request signed by a session key.
    ///
    /// The session key is identified by `session_key_id`, and the
    /// `signature` is a base58-encoded ed25519 signature over `msg.hash()`.
    /// Only "safe" ops are allowed: AddExtension, RemoveExtension, Custom.
    #[payable]
    fn w_execute_session(
        &mut self,
        msg: RequestMessage,
