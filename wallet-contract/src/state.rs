...
/// A session key allowing limited, temporary access to execute safe ops.
#[near(serializers = [borsh, json])]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SessionKey {
    /// ed25519 public key in NEAR format (e.g. "ed25519:base58...")
    pub public_key: String,
    /// Block timestamp (nanoseconds) when this session key was created
    pub created_at: u64,
    /// Block timestamp (nanoseconds) when this session key expires
    pub expires_at: u64,
}
...
pub struct State<PubKey> {
    ...
    /// Session keys for temporary, limited access.
    /// Migrated from old state: defaults to empty if missing during deserialization.
    #[borsh(
        serialize_with = "serialize_session_keys",
        deserialize_with = "deserialize_session_keys",
    )]
    pub session_keys: BTreeMap<String, SessionKey>,
}

fn serialize_session_keys<W: near_sdk::borsh::io::Write>(
    map: &BTreeMap<String, SessionKey>,
    writer: &mut W,
) -> near_sdk::borsh::io::Result<()> {
    near_sdk::borsh::BorshSerialize::serialize(map, writer)
}

fn deserialize_session_keys<R: near_sdk::borsh::io::Read>(
    reader: &mut R,
) -> near_sdk::borsh::io::Result<BTreeMap<String, SessionKey>> {
    // Try to deserialize; if it fails (old state without session_keys),
    // return empty map instead of erroring
